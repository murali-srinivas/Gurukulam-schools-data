...
* **Outsourcing Attendance Module**: Created a dedicated **Attendance of Out Sourcing Teachers** panel accessible via a left-hand navigation link in both school and admin login dashboards. Permits logging monthly attendance records specifying Month (Year-Month date picker), Employee Name, Designation, APCOS ID (retrieved dynamically from the employee's Staffing Particulars record), Days Present, and Remarks. The Employee Name dropdown is populated dynamically with filled Out Sourcing staff records from Staffing Particulars, and Designation and APCOS ID are auto-populated/selected based on the chosen employee. Includes grid views, add/edit/delete operations, real-time searching, school and month filters, and Excel/PDF exporters.
